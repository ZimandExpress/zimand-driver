const translations = {
  de: {
    appName: 'Zimand Driver',
    loginSubtitle: 'Melde dich mit deinem Konto vom Disponenten an',
    email: 'E-Mail',
    password: 'Passwort',
    loginButton: 'Anmelden',
    loggingIn: 'Anmeldung läuft…',
    loginError: 'E-Mail oder Passwort falsch.',
    logout: 'Abmelden',
    tabRides: 'Aufträge',
    tabBidding: 'Ausschreibungen',
    tabEarnings: 'Verdienst',
    tabProfile: 'Profil',
    biddingPlaceholder: 'Verfügbare Aufträge zum direkten Bieten.',
    earningsPlaceholder: 'Deine Beträge, pro Tag und pro Zeitraum.',
    accountLabel: 'Konto',
    typeLabel: 'Typ',
    typeOwnerOperator: 'Eigene Firma',
    typeEmployee: 'Angestellter Fahrer',
    onlineToggle: 'Online für Disponent',
    onlineToggleNote: 'Solange aktiv, sieht der Disponent deine Position live — auch ohne laufende Fahrt',
    noRides: 'Aktuell keine zugewiesenen Fahrten.',
    loadingRides: 'Fahrten werden geladen…',
    pickup: 'Abholung',
    delivery: 'Lieferung',
    statusOpen: 'Offen',
    statusAssigned: 'Zugewiesen',
    statusDone: 'Erledigt',
    statusCancelled: 'Storniert',
    orderRef: 'Auftrag',
    back: 'Zurück',
    cargoLabel: 'Fracht',
    weightLabel: 'Gewicht',
    dimsLabel: 'Maße',
    referenceLabel: 'Referenz',
    kmLabel: 'Entfernung',
    startDriving: 'Losfahren',
    arrived: 'Angekommen',
    confirmStep: 'Bestätigung',
    photosLabel: 'Fotos',
    uploadingLabel: 'Wird hochgeladen…',
    uploadFailedError: 'Verbindung fehlgeschlagen — Fotos und Dokumente sind noch da. Bitte erneut versuchen, sobald Sie wieder Empfang haben.',
    takePhoto: 'Foto aufnehmen',
    chooseFromGallery: 'Aus Galerie wählen',
    cmrLabel: 'CMR-Dokument',
    documentsLabel: 'Dokumente',
    docTypeCmr: 'CMR',
    docTypeProtocol: 'Zustellprotokoll',
    docTypeOther: 'Sonstiges',
    addDocument: 'Dokument hinzufügen',
    docGuideHint: 'Tipp: Dokument gut ausleuchten',
    docGuideTitle: 'So fotografieren Sie richtig',
    docGuideText: 'Legen Sie das Dokument flach hin, sorgen Sie für gute Beleuchtung und fotografieren Sie es gerade von oben — alle vier Ecken müssen sichtbar sein.',
    docGuideClose: 'Verstanden',
    celebrationTitle: 'Glückwunsch!',
    celebrationSubtitle: 'Du hast die Lieferung erfolgreich abgeschlossen.',
    celebrationEarned: 'Dein Verdienst wurde deinem Konto gutgeschrieben.',
    celebrationCta: 'Weiter so! Du machst einen großartigen Job!',
    menuVehicles: 'Meine Fahrzeuge',
    noVehicles: 'Noch keine Fahrzeuge hinterlegt.',
    addVehicleTitle: 'Fahrzeug hinzufügen',
    vehicleModelPlaceholder: 'Modell (z. B. Caddy, Sprinter)',
    vehiclePlatePlaceholder: 'Kennzeichen',
    vehicleYearPlaceholder: 'Baujahr',
    addVehicleButton: '+ Hinzufügen',
    radiusFilterLabel: 'Umkreis:',
    radiusFilterAll: 'Alle',
    radiusFilterEmpty: 'Keine Aufträge in diesem Umkreis — versuchen Sie einen größeren Radius.',
    pushToggle: 'Push-Benachrichtigungen',
    pushToggleNote: 'Werde benachrichtigt, sobald ein Auftrag verfügbar ist oder storniert wird — auch bei gesperrtem Bildschirm',
    signerNameLabel: 'Name des Empfängers',
    signerNameLabelPickup: 'Name der abgebenden Person',
    signerNameLabelDelivery: 'Name des Empfängers',
    signerNamePlaceholder: 'Vor- und Nachname',
    signatureLabel: 'Unterschrift',
    uploadCmr: 'CMR hochladen',
    confirmPickup: 'Abholung bestätigen',
    confirmDelivery: 'Lieferung bestätigen',
    sectionActive: 'Aktive Fahrten',
    sectionHistory: 'Verlauf · Erledigte Fahrten',
    noActiveRides: 'Aktuell keine aktiven Fahrten.',
    priceLabel: 'Preis',
    viewDetails: 'Details ansehen',
    changePhoto: 'Foto ändern',
    welcomeSetPassword: 'Willkommen! Lege dein Passwort fest, um fortzufahren.',
    newPassword: 'Neues Passwort',
    confirmPassword: 'Passwort bestätigen',
    setPasswordButton: 'Passwort speichern',
    passwordTooShort: 'Das Passwort muss mindestens 8 Zeichen haben.',
    passwordsDontMatch: 'Die Passwörter stimmen nicht überein.',
    forgotPassword: 'Passwort vergessen?',
    forgotPasswordSub: 'Gib deine E-Mail-Adresse ein — wir senden dir einen Link zum Zurücksetzen.',
    sendResetLink: 'Link senden',
    resetLinkSent: 'E-Mail gesendet! Prüfe dein Postfach (auch den Spam-Ordner).',
    backToLogin: '← Zurück zur Anmeldung',
    installPromptAndroid: 'Installiere Zimand Driver auf deinem Startbildschirm.',
    installPromptIOS: 'Zum Installieren: Teilen-Symbol antippen, dann "Zum Home-Bildschirm".',
    installButton: 'Installieren',
    earningsWeekLabel: 'Diese Woche · reset jeden Montag',
    earningsPeriod: 'Zeitraum',
    earningsThisWeek: 'Diese Woche',
    earningsPreviousWeeks: 'Vorherige Wochen',
    vehicleLabel: 'Fahrzeug',
    autoAssignHeading: 'Automatische Zuweisung',
    autoAssignToggle: 'Gewonnene Aufträge automatisch zuweisen',
    autoAssignToggleNote: 'Gewonnene Aufträge werden automatisch dir selbst zugewiesen. Danach kannst du sie im Menü einem anderen Fahrer übergeben.',
    defaultDriverLabel: 'Standard-Fahrer',
    defaultDriverNone: 'Kein Standard',
    needsAssignmentHeading: 'Fahrer zuweisen',
    assignDriverLabel: 'Fahrer auswählen',
    assignDriverButton: 'Zuweisen',
    reassignLabel: 'An anderen Fahrer übergeben',
    reassignButton: 'Übergeben',
    callButton: 'Anrufen',
    fixedDeliveryBadge: 'Feste Lieferzeit',
    fixedPickupBadge: 'Feste Abholzeit',
    loadHelpBadge: 'Verladehilfe',
    unloadHelpBadge: 'Entladehilfe',
    neutralDeliveryBadge: 'Neutrale Zustellung',
    navigateButton: 'Navigation',
    enRouteLabel: 'Unterwegs',
    pickedUpLabel: 'Abgeholt',
    returnLabel: 'Rückfahrt',
    altTimeLabel: 'Alternative Zeit',
    deliveredLabel: 'Zugestellt',
    optionalLabel: 'optional',
    doneLabel: 'Fertig',
    menuCompleted: 'Abgeschlossen',
    editBid: 'Bearbeiten',
    withdrawBid: 'Zurückziehen',
    orOwnOffer: 'oder eigenes Angebot',
    pickupWindowNote: 'Abholzeitfenster laut Auftrag',
    canRespectInterval: 'Kann dieses Zeitfenster einhalten',
    fromLabel: 'Von',
    toLabel: 'Bis',
    messageToDispatcher: 'Nachricht an Disponent (optional)',
    submitBid: 'Gebot abgeben',
    bidFootnote: 'Pflicht bei Abholung & Zustellung: Name, Unterschrift & Fotos.',
    todayBadge: 'HEUTE',
    newBadge: 'NEU',
    tomorrowBadge: 'MORGEN',
    bidPlaced: 'Geboten',
    notesLabel: 'Hinweise',
    menuOffers: 'Meine Angebote',
    menuNotAccepted: 'Nicht angenommen',
    noOffersPending: 'Aktuell keine offenen Angebote.',
    noLostBids: 'Keine abgelehnten Angebote.',
    cancelledLabel: 'Auftrag storniert',
    compensationLabel: 'Entschädigung',
    newOrderAlert: 'Neuer Auftrag verfügbar',
    tabAvailable: 'Verfügbar',
    tabMine: 'Meine Aufträge',
    tabDoneTab: 'Abgeschlossen',
    filter: 'Filter',
    sortLabel: 'Sortieren',
    sortNewest: 'Neuestes zuerst',
    sortOldest: 'Ältestes zuerst',
    comingSoon: 'Demnächst verfügbar',
    biddingComingSoon: 'Direktes Bieten kommt bald — verfügbare Aufträge werden hier erscheinen.',

    // --- Upload-Warteschlange / Offline (Batch 1) ---
    retryLabel: 'Erneut versuchen',
    offlineSyncNote: 'Offline – Daten werden automatisch synchronisiert',
    allFilesSynced: 'Alle Dateien synchronisiert',
    filesPendingSync: '{n} Dateien warten auf Synchronisierung',
    filesFailedSync: '{n} Dateien fehlgeschlagen – zum Wiederholen antippen',
    savedLocallyNote: 'Lokal gespeichert – geht nicht verloren',
    waitingForUploads: 'Dateien werden hochgeladen…',
    confirmSavedOffline: 'Lokal gespeichert. Wird gesendet, sobald wieder Netz da ist.',
    photoTooLarge: 'Foto zu groß.',
    photoUnsupported: 'Dateiformat nicht unterstützt.',

    // --- Operative Auftragsphasen ---
    stepLabel: 'Schritt',
    stageAssigned: 'Auftrag erhalten',
    stageToPickup: 'Unterwegs zur Abholung',
    stageAtPickup: 'An Abholung angekommen',
    stagePickupDone: 'Abholung abgeschlossen',
    stageToDelivery: 'Unterwegs zur Zustellung',
    stageAtDelivery: 'An Zustellung angekommen',
    stageReturnReady: 'Rückfahrt bereit',
    stageReturnToPickup: 'Rückfahrt · unterwegs zur Abholung',
    stageReturnAtPickup: 'Rückfahrt · an Abholung angekommen',
    stageReturnPickupDone: 'Rückfahrt · Abholung abgeschlossen',
    stageReturnToDelivery: 'Rückfahrt · unterwegs zur Zustellung',
    stageReturnAtDelivery: 'Rückfahrt · an Zustellung angekommen',

    // --- Rückgängig ---
    undoHint: 'Versehentlich getippt?',
    undoLabel: 'Rückgängig',
    undoFailed: 'Rückgängig machen nicht möglich. Bitte Disponent kontaktieren.',
    undoTooLate: 'Zeitfenster abgelaufen. Bitte Disponent kontaktieren.',

    // --- ETA an den Kunden ---
    etaButton: 'Ankunftszeit an Kunden senden',
    etaButtonAgain: 'Ankunftszeit aktualisieren',
    etaTitle: 'Ankunftszeit senden',
    etaSubtitlePickup: 'Der Kunde erhält eine E-Mail mit Ihrem Namen und dem Zeitfenster, in dem Sie zur Abholung eintreffen.',
    etaSubtitleDelivery: 'Der Kunde erhält eine E-Mail mit Ihrem Namen und dem Zeitfenster, in dem Sie die Sendung zustellen.',
    etaDateLabel: 'Datum',
    etaFromLabel: 'Von',
    etaToLabel: 'Bis',
    etaSharePhone: 'Meine Telefonnummer mitsenden',
    etaSharePhoneNote: 'Der Kunde kann Sie dann direkt anrufen.',
    etaPhonePlaceholder: 'Telefonnummer',
    etaSendButton: 'An Kunden senden',
    etaSentToast: 'Ankunftszeit gesendet ({n}/3)',
    etaAlreadySent: 'Bereits {n}× gesendet. Maximal 3× pro Etappe.',
    etaBadWindow: 'Bitte ein gültiges Zeitfenster wählen.',
    etaLimitReached: 'Maximal 3 Benachrichtigungen pro Etappe erreicht.',
    etaNoRecipient: 'Für diesen Auftrag ist keine Kunden-E-Mail hinterlegt.',
    etaFailed: 'Ankunftszeit konnte nicht gesendet werden.',

    // --- Verdienst im Menü ---
    earningsMonthLabel: 'Diesen Monat',
    earningsTodayLabel: 'Heute',
    earningsRidesCount: '{m} Aufträge · {d} heute',
    earningsMonthPeriodLabel: 'Dieser Monat',
    earningsThisMonth: 'Dieser Monat',
    earningsPreviousMonths: 'Frühere Monate',

    // --- Dokument fotografieren ---
    docChooseType: 'Welches Dokument?',
    docOpenCamera: '📷 Kamera öffnen',
    docFromGallery: '🖼️ Aus Galerie wählen',
    docGuideCmrText: 'Legen Sie den CMR-Frachtbrief flach hin und fotografieren Sie die ganze Seite von oben. Stempel und Unterschrift müssen lesbar sein.',
    docGuideProtocolText: 'Fotografieren Sie das vollständige Zustellprotokoll von oben. Die Unterschrift des Empfängers muss klar erkennbar sein.',
    docGuideOtherText: 'Fotografieren Sie das gesamte Dokument von oben, flach und vollständig im Bild.',
    docRuleCorners: 'Alle vier Ecken im Bild',
    docRuleFlat: 'Blatt flach hinlegen, nicht in der Hand halten',
    docRuleLight: 'Kein Schatten, kein direktes Gegenlicht',
    docRuleReadable: 'Text, Stempel und Unterschrift lesbar',
    qChecking: 'Foto wird geprüft…',
    qGood: 'Foto sieht gut aus.',
    qProblemTitle: 'Foto könnte unlesbar sein:',
    qBlurry: 'unscharf oder verwackelt',
    qDark: 'zu dunkel',
    qBright: 'überbelichtet — zu viel Licht',
    qFlat: 'zu kontrastarm — Text kaum erkennbar',
    qSmall: 'Auflösung zu niedrig',
    qRetake: '📷 Neu aufnehmen',
    qUse: 'Foto verwenden',
    qUseAnyway: 'Trotzdem verwenden',

    // --- Gegenangebot vom Disponenten ---
    counterTitle: 'Gegenangebot vom Disponenten',
    counterAccept: '✓ Preis annehmen',
    counterReject: 'Ablehnen',
    counterRejectedNote: 'Sie haben das Gegenangebot abgelehnt. Der Disponent wurde informiert. Sie können jetzt einen neuen Preis senden.',
    counterFailed: 'Aktion fehlgeschlagen. Bitte erneut versuchen.',

    // --- Problem melden / Wartezeit ---
    waitingTime: 'Wartezeit',
    incidentButton: 'Problem melden',
    incidentTitle: 'Problem melden',
    incidentSubtitle: 'Der Auftrag bleibt offen. Der Disponent wird informiert und entscheidet, wie es weitergeht.',
    incidentCommentPlaceholder: 'Was ist passiert? (optional)',
    incidentSend: 'Problem melden',
    incidentSending: 'Wird gesendet…',
    incidentSaved: 'Problem gemeldet. Der Disponent wurde informiert.',
    incidentFailed: 'Meldung fehlgeschlagen. Bitte erneut versuchen.',
    incident_empfaenger_nicht_vor_ort: 'Empfänger nicht vor Ort',
    incident_empfaenger_nicht_erreichbar: 'Empfänger nicht erreichbar',
    incident_annahme_verweigert: 'Annahme verweigert',
    incident_wartezeit: 'Wartezeit',
    incident_adresse_nicht_gefunden: 'Adresse nicht gefunden',
    incident_adresse_falsch: 'Adresse falsch',
    incident_zufahrt_nicht_moeglich: 'Zufahrt nicht möglich',
    incident_keine_entladehilfe: 'Keine Entladehilfe',
    incident_ware_beschaedigt: 'Ware beschädigt',
    incident_dokument_fehlt: 'Dokument fehlt',
    incident_sonstiges: 'Sonstiges',

    // --- Dokumentenzustellung: Zustellart ---
    deliveryMethodLabel: 'Wie wurde zugestellt?',
    deliveryMethodBriefkasten: 'In den Briefkasten gelegt',
    deliveryMethodBriefkastenNote: 'Namensschild und Adresse vor Ort geprüft — Fotos sind der Nachweis, keine Unterschrift nötig.',
    deliveryMethodPersoenlich: 'Persönlich an den Empfänger übergeben',
    deliveryMethodPersoenlichNote: 'Name und Unterschrift des Empfängers erforderlich.',
    deliveryMethodImpossible: 'Zustellung war nicht möglich',
    directAwardAlert: 'Neuer Auftrag direkt für Sie',
    locationNoticeTitle: 'Bitte direkt vor Ort bestätigen',
    locationNoticeBody: 'Beim Bestätigen wird Ihr Standort einmalig gespeichert. Er ist bei Briefkasten-Zustellungen der einzige Nachweis, dass Sie an der Adresse waren — bestätigen Sie daher am Briefkasten, nicht erst im Auto.',
    deliveryMethodImpossibleNote: 'Der Disponent wird informiert und klärt es mit dem Kunden. Bitte warten Sie auf seine Anweisung.',
    waitingForDispatcher: 'Warten auf Anweisung vom Disponenten',
    waitingForDispatcherNote: 'Ihre Meldung ist angekommen. Der Disponent klärt es mit dem Kunden und meldet sich hier. Die Zustellung kann bis dahin nicht bestätigt werden.',
    dispatcherInstruction: 'Anweisung vom Disponenten',
    dispatcherInstructionAck: 'Verstanden',
  },
  en: {
    appName: 'Zimand Driver',
    loginSubtitle: 'Sign in with the account provided by dispatch',
    email: 'Email',
    password: 'Password',
    loginButton: 'Sign in',
    loggingIn: 'Signing in…',
    loginError: 'Wrong email or password.',
    logout: 'Sign out',
    tabRides: 'Orders',
    tabBidding: 'Bidding',
    tabEarnings: 'Earnings',
    tabProfile: 'Profile',
    biddingPlaceholder: 'Available orders to bid on directly.',
    earningsPlaceholder: 'Your amounts, per day and per period.',
    accountLabel: 'Account',
    typeLabel: 'Type',
    typeOwnerOperator: 'Owner-operator',
    typeEmployee: 'Employee driver',
    onlineToggle: 'Online for dispatch',
    onlineToggleNote: 'While active, dispatch sees your position live — even without an ongoing ride',
    noRides: 'No assigned rides right now.',
    loadingRides: 'Loading rides…',
    pickup: 'Pickup',
    delivery: 'Delivery',
    statusOpen: 'Open',
    statusAssigned: 'Assigned',
    statusDone: 'Done',
    statusCancelled: 'Cancelled',
    orderRef: 'Order',
    back: 'Back',
    cargoLabel: 'Cargo',
    weightLabel: 'Weight',
    dimsLabel: 'Dimensions',
    referenceLabel: 'Reference',
    kmLabel: 'Distance',
    startDriving: 'Start driving',
    arrived: 'Arrived',
    confirmStep: 'Confirmation',
    photosLabel: 'Photos',
    uploadingLabel: 'Uploading…',
    uploadFailedError: "Connection failed — your photos and documents are still here. Please try again once you're back online.",
    takePhoto: 'Take photo',
    chooseFromGallery: 'Choose from gallery',
    cmrLabel: 'CMR document',
    documentsLabel: 'Documents',
    docTypeCmr: 'CMR',
    docTypeProtocol: 'Delivery protocol',
    docTypeOther: 'Other',
    addDocument: 'Add document',
    docGuideHint: 'Tip: good lighting matters',
    docGuideTitle: 'How to photograph it right',
    docGuideText: 'Lay the document flat, make sure the lighting is good, and photograph it straight from above — all four corners must be visible.',
    docGuideClose: 'Got it',
    celebrationTitle: 'Congratulations!',
    celebrationSubtitle: 'You successfully completed the delivery.',
    celebrationEarned: 'Your earnings have been credited to your account.',
    celebrationCta: 'Keep it up! Great job!',
    menuVehicles: 'My Vehicles',
    noVehicles: 'No vehicles added yet.',
    addVehicleTitle: 'Add vehicle',
    vehicleModelPlaceholder: 'Model (e.g. Caddy, Sprinter)',
    vehiclePlatePlaceholder: 'License plate',
    vehicleYearPlaceholder: 'Year',
    addVehicleButton: '+ Add',
    radiusFilterLabel: 'Radius:',
    radiusFilterAll: 'All',
    radiusFilterEmpty: 'No orders in this radius — try a larger range.',
    pushToggle: 'Push notifications',
    pushToggleNote: 'Get notified when an order becomes available or is cancelled — even with the screen locked',
    signerNameLabel: "Recipient's name",
    signerNameLabelPickup: 'Name of person handing over',
    signerNameLabelDelivery: "Recipient's name",
    signerNamePlaceholder: 'First and last name',
    signatureLabel: 'Signature',
    uploadCmr: 'Upload CMR',
    confirmPickup: 'Confirm pickup',
    confirmDelivery: 'Confirm delivery',
    sectionActive: 'Active rides',
    sectionHistory: 'History · Completed rides',
    noActiveRides: 'No active rides right now.',
    priceLabel: 'Price',
    viewDetails: 'View details',
    changePhoto: 'Change photo',
    welcomeSetPassword: 'Welcome! Set your password to continue.',
    newPassword: 'New password',
    confirmPassword: 'Confirm password',
    setPasswordButton: 'Save password',
    passwordTooShort: 'Password must be at least 8 characters.',
    passwordsDontMatch: "Passwords don't match.",
    forgotPassword: 'Forgot password?',
    forgotPasswordSub: 'Enter your email — we\'ll send you a reset link.',
    sendResetLink: 'Send link',
    resetLinkSent: 'Email sent! Check your inbox (and spam folder).',
    backToLogin: '← Back to login',
    installPromptAndroid: 'Install Zimand Driver on your home screen.',
    installPromptIOS: 'To install: tap Share, then "Add to Home Screen".',
    installButton: 'Install',
    earningsWeekLabel: 'This week · resets every Monday',
    earningsPeriod: 'Period',
    earningsThisWeek: 'This week',
    earningsPreviousWeeks: 'Previous weeks',
    vehicleLabel: 'Vehicle',
    autoAssignHeading: 'Automatic Assignment',
    autoAssignToggle: 'Automatically assign won orders',
    autoAssignToggleNote: 'Won orders get assigned to you automatically. You can then hand them to another driver from the menu.',
    defaultDriverLabel: 'Default driver',
    defaultDriverNone: 'No default',
    needsAssignmentHeading: 'Assign a driver',
    assignDriverLabel: 'Choose driver',
    assignDriverButton: 'Assign',
    reassignLabel: 'Hand off to another driver',
    reassignButton: 'Hand off',
    callButton: 'Call',
    fixedDeliveryBadge: 'Fixed delivery time',
    fixedPickupBadge: 'Fixed pickup time',
    loadHelpBadge: 'Loading help',
    unloadHelpBadge: 'Unloading help',
    neutralDeliveryBadge: 'Neutral delivery',
    navigateButton: 'Navigate',
    enRouteLabel: 'On the way',
    pickedUpLabel: 'Picked up',
    returnLabel: 'Return',
    altTimeLabel: 'Alternative time',
    deliveredLabel: 'Delivered',
    optionalLabel: 'optional',
    doneLabel: 'Done',
    menuCompleted: 'Completed',
    editBid: 'Edit',
    withdrawBid: 'Withdraw',
    orOwnOffer: 'or your own offer',
    pickupWindowNote: 'Pickup window per order',
    canRespectInterval: 'Can meet this time window',
    fromLabel: 'From',
    toLabel: 'To',
    messageToDispatcher: 'Message to dispatcher (optional)',
    submitBid: 'Submit bid',
    bidFootnote: 'Required at pickup & delivery: name, signature & photos.',
    todayBadge: 'TODAY',
    newBadge: 'NEW',
    tomorrowBadge: 'TOMORROW',
    bidPlaced: 'Bid placed',
    notesLabel: 'Notes',
    menuOffers: 'My Offers',
    menuNotAccepted: 'Not Accepted',
    noOffersPending: 'No pending offers right now.',
    noLostBids: 'No declined offers.',
    cancelledLabel: 'Order cancelled',
    compensationLabel: 'Compensation',
    newOrderAlert: 'New order available',
    tabAvailable: 'Available',
    tabMine: 'My Orders',
    tabDoneTab: 'Completed',
    filter: 'Filter',
    sortLabel: 'Sort',
    sortNewest: 'Newest first',
    sortOldest: 'Oldest first',
    comingSoon: 'Coming soon',
    biddingComingSoon: 'Direct bidding is coming soon — available orders will appear here.',

    // --- Upload queue / offline (batch 1) ---
    retryLabel: 'Retry',
    offlineSyncNote: 'Offline – data will sync automatically',
    allFilesSynced: 'All files synced',
    filesPendingSync: '{n} files waiting to sync',
    filesFailedSync: '{n} files failed – tap to retry',
    savedLocallyNote: 'Saved locally – will not be lost',
    waitingForUploads: 'Uploading files…',
    confirmSavedOffline: 'Saved locally. Will be sent as soon as you are back online.',
    photoTooLarge: 'Photo too large.',
    photoUnsupported: 'File format not supported.',

    // --- Operational order stages ---
    stepLabel: 'Step',
    stageAssigned: 'Order received',
    stageToPickup: 'On the way to pickup',
    stageAtPickup: 'Arrived at pickup',
    stagePickupDone: 'Pickup completed',
    stageToDelivery: 'On the way to delivery',
    stageAtDelivery: 'Arrived at delivery',
    stageReturnReady: 'Return trip ready',
    stageReturnToPickup: 'Return · on the way to pickup',
    stageReturnAtPickup: 'Return · arrived at pickup',
    stageReturnPickupDone: 'Return · pickup completed',
    stageReturnToDelivery: 'Return · on the way to delivery',
    stageReturnAtDelivery: 'Return · arrived at delivery',

    // --- Undo ---
    undoHint: 'Tapped by mistake?',
    undoLabel: 'Undo',
    undoFailed: 'Could not undo. Please contact dispatch.',
    undoTooLate: 'Time window expired. Please contact dispatch.',

    // --- ETA to the client ---
    etaButton: 'Send arrival time to client',
    etaButtonAgain: 'Update arrival time',
    etaTitle: 'Send arrival time',
    etaSubtitlePickup: 'The client gets an email with your name and the time window in which you will arrive for pickup.',
    etaSubtitleDelivery: 'The client gets an email with your name and the time window in which you will deliver.',
    etaDateLabel: 'Date',
    etaFromLabel: 'From',
    etaToLabel: 'To',
    etaSharePhone: 'Include my phone number',
    etaSharePhoneNote: 'The client will be able to call you directly.',
    etaPhonePlaceholder: 'Phone number',
    etaSendButton: 'Send to client',
    etaSentToast: 'Arrival time sent ({n}/3)',
    etaAlreadySent: 'Already sent {n}×. Maximum 3× per leg.',
    etaBadWindow: 'Please choose a valid time window.',
    etaLimitReached: 'Maximum of 3 notifications per leg reached.',
    etaNoRecipient: 'No client email is stored for this order.',
    etaFailed: 'Could not send arrival time.',

    // --- Earnings in the menu ---
    earningsMonthLabel: 'This month',
    earningsTodayLabel: 'Today',
    earningsRidesCount: '{m} orders · {d} today',
    earningsMonthPeriodLabel: 'This month',
    earningsThisMonth: 'This month',
    earningsPreviousMonths: 'Previous months',

    // --- Photographing a document ---
    docChooseType: 'Which document?',
    docOpenCamera: '📷 Open camera',
    docFromGallery: '🖼️ Choose from gallery',
    docGuideCmrText: 'Lay the CMR consignment note flat and photograph the whole page from above. Stamp and signature must be readable.',
    docGuideProtocolText: 'Photograph the complete delivery protocol from above. The recipient signature must be clearly visible.',
    docGuideOtherText: 'Photograph the whole document from above, flat and fully in frame.',
    docRuleCorners: 'All four corners in the frame',
    docRuleFlat: 'Lay the sheet flat, do not hold it',
    docRuleLight: 'No shadow, no direct backlight',
    docRuleReadable: 'Text, stamp and signature readable',
    qChecking: 'Checking photo…',
    qGood: 'Photo looks good.',
    qProblemTitle: 'This photo may be unreadable:',
    qBlurry: 'blurry or shaken',
    qDark: 'too dark',
    qBright: 'overexposed — too much light',
    qFlat: 'too little contrast — text barely visible',
    qSmall: 'resolution too low',
    qRetake: '📷 Retake',
    qUse: 'Use photo',
    qUseAnyway: 'Use anyway',

    // --- Counter offer from dispatch ---
    counterTitle: 'Counter offer from dispatch',
    counterAccept: '✓ Accept price',
    counterReject: 'Decline',
    counterRejectedNote: 'You declined the counter offer. Dispatch has been informed. You can now send a new price.',
    counterFailed: 'Action failed. Please try again.',

    // --- Report a problem / waiting time ---
    waitingTime: 'Waiting time',
    incidentButton: 'Report a problem',
    incidentTitle: 'Report a problem',
    incidentSubtitle: 'The order stays open. Dispatch is informed and decides what happens next.',
    incidentCommentPlaceholder: 'What happened? (optional)',
    incidentSend: 'Report problem',
    incidentSending: 'Sending…',
    incidentSaved: 'Problem reported. Dispatch has been informed.',
    incidentFailed: 'Could not send the report. Please try again.',
    incident_empfaenger_nicht_vor_ort: 'Recipient not on site',
    incident_empfaenger_nicht_erreichbar: 'Recipient unreachable',
    incident_annahme_verweigert: 'Acceptance refused',
    incident_wartezeit: 'Waiting time',
    incident_adresse_nicht_gefunden: 'Address not found',
    incident_adresse_falsch: 'Wrong address',
    incident_zufahrt_nicht_moeglich: 'No access',
    incident_keine_entladehilfe: 'No unloading help',
    incident_ware_beschaedigt: 'Goods damaged',
    incident_dokument_fehlt: 'Document missing',
    incident_sonstiges: 'Other',

    // --- Document delivery: method ---
    deliveryMethodLabel: 'How was it delivered?',
    deliveryMethodBriefkasten: 'Placed in the letterbox',
    deliveryMethodBriefkastenNote: 'Name plate and address checked on site — photos are the proof, no signature needed.',
    deliveryMethodPersoenlich: 'Handed over personally',
    deliveryMethodPersoenlichNote: 'Recipient name and signature required.',
    deliveryMethodImpossible: 'Delivery was not possible',
    directAwardAlert: 'New order assigned directly to you',
    locationNoticeTitle: 'Please confirm while still on site',
    locationNoticeBody: 'Your location is recorded once when you confirm. For letterbox deliveries it is the only proof that you were at the address — so confirm at the letterbox, not back in the car.',
    deliveryMethodImpossibleNote: 'Dispatch will be informed and will clarify with the client. Please wait for their instruction.',
    waitingForDispatcher: 'Waiting for instruction from dispatch',
    waitingForDispatcherNote: 'Your report has arrived. Dispatch is clarifying with the client and will reply here. Delivery cannot be confirmed until then.',
    dispatcherInstruction: 'Instruction from dispatch',
    dispatcherInstructionAck: 'Understood',
  },
}

const STORAGE_KEY = 'zimand-driver-lang'

export function getLang() {
  const saved = typeof window !== 'undefined' ? localStorage.getItem(STORAGE_KEY) : null
  return saved && translations[saved] ? saved : 'de'
}

export function setLang(lang) {
  if (typeof window !== 'undefined' && translations[lang]) {
    localStorage.setItem(STORAGE_KEY, lang)
  }
}

export function t(key, lang) {
  const dict = translations[lang] || translations.de
  return dict[key] || translations.de[key] || key
}

export const availableLangs = Object.keys(translations)
